...
def remove_stops(texts, stops):
    for text in texts:
        #print(text) #looking at output
        text = unidecode(text)
        text = re.sub(r"Signed-off-by:.*>","",text)
        text = re.sub(r"\.md","",text)
        text = re.sub(r"\.xhtml", "", text)
        text = re.sub(r"\.yml", "", text)
        text = re.sub(r"\.xml", "", text)
        text = re.sub(r"\.java", "", text)
        text = re.sub(r"\/", " ", text)
        text = re.sub(r"\.*_soapUI_.*", " ", text)
        #text = re.sub(r"\-", " " , text)
        text = re.sub(r"\{", " ", text)
        text = re.sub(r"\}", " ", text)
        text = re.sub(r"\=\>", " ", text)
...
stats = get_stat(gitlog_list)
cleaned_stats = clean_logs(stats)
#print(gitlog_list)
#print(stats)
#print("\n\n")
#print(cleaned_stats)
...
vectors = vectorizer.fit_transform(cleaned_stats)
...
with open ("data/stats_results.txt", "w", encoding="utf-8") as f:
